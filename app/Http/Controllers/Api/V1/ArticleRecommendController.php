...
use App\Repositories\ArticleRecommendRepository;

class ArticleRecommendController extends Controller
{
	 /**
     * @api {get} /article/recommend/37fb591be38db52dd1d5f04b689008f6 推荐资讯列表
     * @apiVersion 0.0.1
     * @apiName Article-Recommend
     * @apiGroup Article
     *
     ...
     *   }
     */
    public function index( ArticleRecommendRepository $repository)
    {
        $article = $repository->ajaxIndex();
		if($article->count()){
			return response()->json(['status_code'=>200,'msg'=>'获取推荐资讯成功！','article'=>$article]);
		}else{
			return response()->json(['status_code'=>404,'msg'=>'获取推荐资讯失败！','article'=>false]);
		}
    }
}
