...
use App\Repositories\ArticleOfferRepository;

class ArticleOfferController extends Controller
{
	 /**
     * @api {get} /article/offer/37fb591be38db52dd1d5f04b689008f6 市场报价列表
     * @apiVersion 0.0.1
     * @apiName Article-Offer
     * @apiGroup Article
     *
     ...
     *   }
     */
    public function index( ArticleOfferRepository $repository)
    {
        $article = $repository->ajaxIndex();
		if($article->count()){
			return response()->json(['status_code'=>200,'msg'=>'获取市场报价成功！','article'=>$article]);
		}else{
			return response()->json(['status_code'=>404,'msg'=>'获取市场报价失败！','article'=>false]);
		}
    }
}
