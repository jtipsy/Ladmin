...
use App\Repositories\ArticleIndustryRepository;

class ArticleIndustryController extends Controller
{
	 /**
     * @api {get} /article/industry/37fb591be38db52dd1d5f04b689008f6 行业资讯列表
     * @apiVersion 0.0.1
     * @apiName Article-Industry
     * @apiGroup Article
     *
     ...
     *   }
     */
    public function index( ArticleIndustryRepository $repository)
    {
		//page=1 输入参数分页显示数据
        $article = $repository->ajaxIndex();
		if($article->count()){
			return response()->json(['status_code'=>200,'msg'=>'获取行业资讯成功！','article'=>$article]);
		}else{
			return response()->json(['status_code'=>404,'msg'=>'获取行业资讯失败！','article'=>false]);
		}		
